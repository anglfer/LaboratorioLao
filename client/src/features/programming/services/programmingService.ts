...
export class ProgrammingService {
  ...
  async getProgramaciones(
    ...
    if (filters?.brigadistaId)
      params.append("brigadistaId", filters.brigadistaId.toString());
    if (filters?.estado) params.append("estado", filters.estado);
    if (filters?.claveObra) params.append("claveObra", filters.claveObra);

    const response = await fetch(
      `${API_BASE}/programaciones?${params.toString()}`,
    );
    if (!response.ok) {
  ...
  async getBrigadistasDisponibles(
    ...
    const response = await fetch(
      `${API_BASE}/brigadistas/disponibles?${params.toString()}`,
    );
    if (!response.ok) {
  ...
  async getVehiculosDisponibles(
    ...
    const params = new URLSearchParams({ fecha, hora });
    const response = await fetch(
      `${API_BASE}/vehiculos/disponibles?${params.toString()}`,
    );
    if (!response.ok) {
  ...
  // ============ ESTADÍSTICAS ============
  async getEstadisticasSemana(
    fechaInicio: string,
  ): Promise<EstadisticasSemana> {
  ...
  async getDatosGraficaSemana(
    fechaInicio: string,
  ): Promise<DatosGraficaSemana[]> {
    const params = new URLSearchParams({ fechaInicio });
    const response = await fetch(
      `${API_BASE}/dashboard/grafica?${params.toString()}`,
    );
    if (!response.ok) {
  ...
  async reprogramarActividad(
    id: number,
    nuevaFecha: string,
    nuevaHora: string,
  ): Promise<Programacion> {
    const response = await fetch(
      `${API_BASE}/programaciones/${id}/reprogramar`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  async getProgramacionesBrigadista(
    brigadistaId: number,
    fecha?: string,
  ): Promise<Programacion[]> {
    const params = new URLSearchParams({
      brigadistaId: brigadistaId.toString(),
    });
    if (fecha) params.append("fecha", fecha);

    const response = await fetch(
      `${API_BASE}/programaciones/brigadista?${params.toString()}`,
    );
    if (!response.ok) {
  ...
  async getBrigadistaProgramaciones(
    filters?: ProgramacionFilters,
  ): Promise<Programacion[]> {
    const params = new URLSearchParams();
