    ...
    <div className="max-w-3xl mx-auto bg-white p-8 print:p-0 print:bg-white">
      ...
      <div className="border-b pb-4 mb-6 text-center">
        ...
        <div className="text-lg font-bold text-gray-800 mt-2">
          Clave de Obra:{" "}
          <span className="text-green-700">{budget.budgetCode}</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <div>
            <b>Cliente:</b> {budget.clientName}
          </div>
          <div>
            <b>Contacto:</b> {budget.clientContact || "-"}
          </div>
          <div>
            <b>Teléfono:</b> {budget.clientPhone || "-"}
          </div>
          <div>
            <b>Correo:</b> {budget.clientEmail || "-"}
          </div>
          <div>
            <b>Descripción del Proyecto:</b> {budget.projectDescription || "-"}
          </div>
          <div>
            <b>Ubicación del Proyecto:</b> {budget.projectLocation || "-"}
          </div>
          <div>
            <b>Ubicación:</b> {budget.location || "-"}
          </div>
        </div>
        <div>
          <div>
            ...
            {budget.requestDate
              ? new Date(budget.requestDate).toLocaleDateString("es-MX")
              : "-"}
          </div>
          <div>
            <b>Fecha de Estimación:</b>{" "}
          ...
          {items.length === 0 ? (
            <tr>
              <td colSpan={6} className="text-center text-gray-400 py-8">
                No hay conceptos guardados para este presupuesto.
              </td>
            </tr>
          ) : (
            items.map((item, idx) => (
              ...
              >
                ...
                <td className="border px-2 py-1">
                  {item.description || (
                    <span className="text-red-500">(Sin descripción)</span>
                  )}
                </td>
                <td className="border px-2 py-1 text-center">
                ...
                <td className="border px-2 py-1 text-right">
                  {item.unitPrice ? (
                    `$${parseFloat(item.unitPrice).toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                    })}`
                  ) : (
                    <span className="text-red-500">-</span>
      ...
      <div className="mb-8">
        ...
        <ul className="list-disc pl-6 text-sm">
          {SYSTEM_CONSTANTS.TERMS_AND_CONDITIONS.map((t, i) => (
            <li key={i}>{t}</li>
          ))}
        </ul>
      </div>
